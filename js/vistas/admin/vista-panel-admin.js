        ...
        <div class="o-contenedor o-pila o-pila--lg" style="padding-top:var(--espaciado-lg);padding-bottom:100px">
          ...
          <div class="o-pila" id="adminSecciones">
            <div class="seccion-admin">
              ...
              <div class="o-pila">${usuarios.map(u => `<div class="tarjeta-capitulo" style="border-left:3px solid ${u.activo ? 'var(--color-exito)' : 'var(--color-error)'}"><div class="o-flecha o-flecha--between"><div><span class="u-fw-600">${window.helpers.escapeHtml(u.nombre_completo)}</span><span class="u-fs-xs u-color-texto-secundario"> @${u.username} · ${u.rol}</span></div><div class="o-flecha" style="gap:4px">${u.rol !== 'owner' ? `<button class="btn-toggle-activo" data-id="${u.id}" data-activo="${!u.activo}" style="font-size:var(--texto-xs);background:none;border:1px solid var(--color-borde);border-radius:var(--radio-sm);cursor:pointer;padding:2px 6px;display:inline-flex;align-items:center;gap:4px">${u.activo ? window.Iconos.render('ban') + ' Desactivar' : window.Iconos.render('circle-check') + ' Activar'}</button>` : ''}</div></div><div class="o-flecha u-mt-1" style="gap:var(--espaciado-xs);flex-wrap:wrap">${['usuario','editor','admin'].filter(r => r !== u.rol).map(r => `<button class="btn-cambiar-rol" data-id="${u.id}" data-rol="${r}" style="font-size:var(--texto-xs);background:var(--color-fondo);border:1px solid var(--color-borde);border-radius:var(--radio-sm);cursor:pointer;padding:2px 6px">Hacer ${r}</button>`).join('')}</div></div>`).join('')}</div>
            </div>
